    ...
    <div className="flex flex-col items-center justify-center h-screen bg-gradient-to-r from-cyan-500 to-blue-500 p-4">
      <div className="max-w-4xl rounded overflow-hidden shadow-lg bg-white p-6">
        <div className="text-center">
          <img className="w-48 h-48 rounded-full mx-auto" src={basics.image} alt={basics.name} />
        ...
        <div className="mt-6">
          ...
          {education.map((edu, index) => (
            <p key={index} className="text-gray-700">{edu.institution} - {edu.area} ({edu.startDate} - {edu.endDate})</p>
          ))}
        </div>
        <div className="mt-6">
          ...
          <div className="flex flex-wrap">
            {skills.map((skill, index) => (
              <span key={index} className="m-1 bg-blue-100 text-blue-800 text-sm font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800">{skill.name}</span>
            ))}
          </div>
        </div>
        <div className="mt-6">
          ...
          {projects.map((project, index) => (
            <a key={index} href={project.url} className="text-blue-500 hover:underline">{project.name}</a>
          ))}
        </div>
      </div>
    </div>
...
/*  <div className="h-screen flex justify-center items-center bg-gradient-to-r from-purple-500 to-blue-500">
      <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white p-4">
        <img className="w-full" src={cv.basics.image}alt={cv.basics.name} />
        <div className="px-6 py-4">
          <div className="font-bold text-xl mb-2">{cv.basics.name}</div>
